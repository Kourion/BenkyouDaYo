...
\begin{itemize}
\item Things that are \ruby{隣}{となり} must be of the same type, caliber, or quality. They must be perceived in some regard as equal. \ruby{隣}{となり} is also used to refer to the nearest object of the same type, regardless of whether the actual distance is near or far.
\item Things that are \ruby{横}{よこ} are right next to each other, size and quality do not matter.
\end{itemize}
\end{document}